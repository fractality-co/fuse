...
namespace Fuse.Editor
{
    ...
    public static class GraphBackground
    {
        ...
        public static void DrawGraphBackground(Rect position, Rect graphExtents, float scale = 1f)
        {
            ...
            DrawGrid(graphExtents, scale);
        }

        private static void DrawGrid(Rect graphExtents, float scale = 1f)
        {
            if (Event.current.type == EventType.Repaint)
            {
                ...
                GL.PushMatrix();
                GL.Begin(1);
                DrawGridLines(graphExtents, 12f * scale, gridMinorColor);
                DrawGridLines(graphExtents, 120f * scale, gridMajorColor);
                GL.End();
                GL.PopMatrix();
            }
        }
        ...
        static class HandleUtility
        {
            ...
            public static void ApplyWireMaterial(CompareFunction zTest = CompareFunction.Always)
            {

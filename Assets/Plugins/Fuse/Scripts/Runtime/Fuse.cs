...
namespace Fuse
{
    ...
    public class Fuse : SingletonBehaviour<Fuse>
    {
        ...
        public static readonly string[] Validation = { "4B67A2", "4D69A5" };
        ...
        private static void AnalyticsPrehook()
        {
        ...
        {
            Application.targetFrameRate = 120; // cap max frame rate to 120hz
            ...
            while (!success)
            {
                ...
                if (_configuration.Loader != null && !restricted)
                {
                ...
                {
                    if (fuseLogo == null || !Validate(fuseLogo))
                    {
                        Logger.Error(
        ...
        private static bool Validate(Sprite sprite)
        {
            ...
            var colors = sprite.texture.GetPixels(sprite.texture.width / 2, sprite.texture.height / 2, 2, 2);
            return colors.Length >= 4 && colors.All(color => Validation.Contains(ColorUtility.ToHtmlStringRGB(color)));
        }
        ...
        private IEnumerator SetState(State state)
        {
            ...
            if (_state != null)
            {
                yield return UnloadAll<ModuleInstance>(_state.Modules.Reverse(), state.Modules.Reverse().ToArray());
                yield return UnloadAll<ContentInstance>(_state.Content.Reverse(), state.Content.Reverse().ToArray());
                yield return UnloadAll<SceneInstance>(_state.Scenes.Reverse(), state.Scenes.Reverse().ToArray());
            }
